-- Update the process_deposit_approval function to NOT deduct from the deposit wallet
-- when a plan_id is present. This ensures the "Deposit Capital" balance reflects the invested funds.
...
BEGIN
  ...
  -- This balance now represents "Deposit Capital" on the dashboard
  UPDATE wallets SET 
  ...
  IF deposit_record.plan_id IS NOT NULL THEN
    ...
    -- This record is what triggers the ROI distribution
    INSERT INTO user_investment_selections (
      user_id,
    ...
    -- REMOVED: Deduction from deposit wallet. 
    -- We now keep it in the deposit wallet so it shows up as "Deposit Capital"
    -- and contributes to the "Total Value" on the dashboard.

    -- Notify user of auto-investment
    INSERT INTO notifications (user_id, title, message, type)
