...
BEGIN
    ...
    FROM (
        SELECT 
            ...
            COALESCE(SUM(CASE WHEN uis.is_active = true THEN uis.amount ELSE 0 END), 0) as active_invested,
            COALESCE((
            ...
            COUNT(DISTINCT CASE WHEN uis.is_active = true THEN uis.user_id END) as active_users,
            COALESCE(SUM(CASE WHEN uis.is_active = true THEN uis.amount * (io.interest_rate / 100) ELSE 0 END), 0) as projected_monthly_payout
    ...
    SELECT json_build_object(
        'total_active_volume', COALESCE(SUM(uis.amount), 0),
        'total_projected_monthly', COALESCE(SUM(uis.amount * (io.interest_rate / 100)), 0)
    ) INTO v_total_stats
