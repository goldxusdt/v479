-- Corrected distribute_daily_roi with proper user-based grouping
...
BEGIN
    ...
    FOR v_user_record IN 
        SELECT DISTINCT p.id, p.last_roi_credit_at
        FROM profiles p
    ...
    LOOP
        ...
        IF v_last_credit IS NOT NULL AND v_now >= v_last_credit + INTERVAL '23 hours 55 minutes' THEN
            ...
            LOOP
                ...
                IF v_roi_amount > 0 THEN
                    ...
                    SET balance = balance + v_roi_amount,
                        updated_at = v_now
                    WHERE user_id = v_user_record.id AND wallet_type = 'roi';
            ...
            VALUES (v_user_record.id, 'ROI Credited', 'Your daily ROI has been credited.', 'roi', v_now);
