...
BEGIN
  ...
  -- Loop through referral chain up to 15 levels
  WHILE current_referrer IS NOT NULL AND current_level <= 15 LOOP
    -- Get commission rate for current level from settings table
    -- Robust key matching to handle inconsistent naming in the database
    SELECT value INTO v_rate_val 
    FROM settings 
    WHERE key = 'level' || current_level || '_commission'
       OR key = 'referral_level' || current_level || '_commission'
       OR key = 'referral_level' || current_level || '_percentage'
    ORDER BY (
      CASE 
        WHEN key = 'level' || current_level || '_commission' THEN 1
        WHEN key = 'referral_level' || current_level || '_commission' THEN 2
        ELSE 3
      END
    ) ASC
    LIMIT 1;
