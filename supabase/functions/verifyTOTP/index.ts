...
serve(async (req) => {
  ...
  try {
    ...
    const { email, userId, otp, totp_code, isBackupCode, setupSecret, purpose } = sanitizeInput(rawBody);

    const targetEmail = email;
    const targetUserId = userId;
    const verificationCode = totp_code || otp;

    if ((!targetEmail && !targetUserId) || !verificationCode) {
      return new Response(JSON.stringify({ error: 'Identification and Code are required' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    let query = supabase.from('profiles').select('mfa_secret, mfa_enabled, id, mfa_locked_until, mfa_failed_attempts, mfa_backup_codes, email, mfa_pending_secret');
